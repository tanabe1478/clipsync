...
│   ├── lib/features/     # auth, clips, share, settings
│   ├── test/             # Unit + Widget tests
│   └── integration_test/ # E2E tests
├── e2e/                  # Playwright E2E tests (desktop)
...
```bash
pnpm test                 # Frontend unit tests (vitest, 36 tests)
pnpm test:e2e             # E2E tests (Playwright, 7 tests)
cargo test --manifest-path src-tauri/Cargo.toml  # Rust tests
cd mobile && flutter test  # Mobile unit + widget tests
```
...
├── 005-flutter-mobile-crossplatform.md
├── 006-riverpod-state-management.md
└── 007-feature-first-flutter-architecture.md
```
...
**ルール（必須 — 違反は禁止）**:
1. `.rs`, `.ts`, `.tsx`, `.sql`, `.dart` ファイルを編集する**前に**、下表の Trigger 列に該当する spec を**必ず Read すること**（hooks でリマインドされるが、hook が動作しなくてもこのルールは有効）
...
| [mobile-app.md](docs/specs/mobile-app.md) | clip.dart, clips_repository.dart, clips_notifier.dart, realtime_clips.dart, auth_notifier.dart, share_handler.dart | Flutter アーキテクチャ、CRUD、Realtime |
| [bug-memory.md](docs/specs/bug-memory.md) | 全ファイル（デバッグ時） | 過去のバグパターンと修正方法 |

### Tier 3: オンデマンド検索 → `docs/adr/`

- `docs/adr/` — 設計判断の経緯（000-007）
